...
function openim::rpc::start() {
    ...
    for ((i = 0; i < ${#OPENIM_RPC_SERVICE_LISTARIES[*]}; i++)); do
        ...
        for ((j = 0; j < ${#OPENIM_RPC_SERVICE_PORTS_ARRAY[@]}; j++)); do
            ...
            # Add delay between service starts to avoid resource contention
            sleep 2
        done
    done
    ...
    # Check ports but don't exit on failure, just log warnings
    openim::log::info "Checking RPC service ports..."
    failed_ports=()
    for port in ${OPENIM_RPC_PORT_TARGETS[@]}; do
        if ! netstat -tuln | grep ":$port " > /dev/null; then
            failed_ports+=($port)
            openim::log::error "Port $port is not listening"
        else
            openim::log::info "Port $port is listening successfully"
        fi
    done

    if [ ${#failed_ports[@]} -gt 0 ]; then
        openim::log::error "Some RPC services failed to start on ports: ${failed_ports[@]}"
        openim::log::error "Check logs for details, but continuing with startup..."
    else
        openim::log::info "All RPC services started successfully"
    fi
