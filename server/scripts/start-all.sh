    ...
    for script_path in "${OPENIM_SERVER_SCRIPT_START_LIST[@]}"; do
        ...
        if [[ -x "$script_path" ]]; then
            ...
            if [[ $? -eq 0 ]]; then
                ...
                openim::log::error "Error executing ${script_path##*/}, but continuing with other services..."
            fi
        else
            openim::log::error "Script ${script_path##*/} is missing or not executable, but continuing..."
        fi
    done
    sleep 0.5
}
...
openim::log::info "\n## Ensuring MessageGateway is running"
# Check if MessageGateway is running, if not, start it
if ! pgrep -f "openim-msggateway" > /dev/null; then
    openim::log::info "MessageGateway not found, starting it..."
    ${START_SCRIPTS_PATH}/openim-msggateway.sh openim::msggateway::start
    if [[ $? -eq 0 ]]; then
        openim::log::info "MessageGateway started successfully."
    else
        openim::log::error "Failed to start MessageGateway, but continuing..."
    fi
else
    openim::log::info "MessageGateway is already running."
fi
