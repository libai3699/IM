...
func (m *msgServer) sendMsgSingleChat(ctx context.Context, req *pbmsg.SendMsgReq) (resp *pbmsg.SendMsgResp, err error) {
	log.ZDebug(ctx, "[diag-send-msg] sendMsgSingleChat enter",
		"opUserID", mcontext.GetOpUserID(ctx), "sendID", req.MsgData.SendID, "recvID", req.MsgData.RecvID,
		"clientMsgID", req.MsgData.ClientMsgID, "contentType", req.MsgData.ContentType)
	if err := m.messageVerification(ctx, req); err != nil {
		log.ZError(ctx, "[diag-send-msg] sendMsgSingleChat messageVerification failed", err,
			"sendID", req.MsgData.SendID, "recvID", req.MsgData.RecvID)
		return nil, err
	}
	isSend := true
	...
	if !isNotification {
		...
		if err != nil {
			log.ZError(ctx, "[diag-send-msg] sendMsgSingleChat modifyMessageByUserMessageReceiveOpt failed", err,
				"recvID", req.MsgData.RecvID, "conversationID", utils.GenConversationIDForSingle(req.MsgData.SendID, req.MsgData.RecvID))
			return nil, err
		}
	}
	if !isSend {
		log.ZDebug(ctx, "[diag-send-msg] sendMsgSingleChat dropped by recv opt (not an error)",
			"recvID", req.MsgData.RecvID, "sendID", req.MsgData.SendID)
		prommetrics.SingleChatMsgProcessFailedCounter.Inc()
		return nil, nil
	} else {
		if err = callbackBeforeSendSingleMsg(ctx, req); err != nil {
			log.ZError(ctx, "[diag-send-msg] sendMsgSingleChat callbackBeforeSendSingleMsg failed", err, "sendID", req.MsgData.SendID)
			return nil, err
		}
		if err := callbackMsgModify(ctx, req); err != nil {
			log.ZError(ctx, "[diag-send-msg] sendMsgSingleChat callbackMsgModify failed", err, "sendID", req.MsgData.SendID)
			return nil, err
		}
		if err := m.MsgDatabase.MsgToMQ(ctx, utils.GenConversationUniqueKeyForSingle(req.MsgData.SendID, req.MsgData.RecvID), req.MsgData); err != nil {
			log.ZError(ctx, "[diag-send-msg] sendMsgSingleChat MsgToMQ failed", err, "sendID", req.MsgData.SendID, "recvID", req.MsgData.RecvID)
			prommetrics.SingleChatMsgProcessFailedCounter.Inc()
			return nil, err
		}
		log.ZDebug(ctx, "[diag-send-msg] sendMsgSingleChat MsgToMQ ok",
			"sendID", req.MsgData.SendID, "recvID", req.MsgData.RecvID, "clientMsgID", req.MsgData.ClientMsgID)
		err = callbackAfterSendSingleMsg(ctx, req)
		if err != nil {
			log.ZError(ctx, "[diag-send-msg] sendMsgSingleChat CallbackAfterSendSingleMsg failed", err, "req", req)
		}
		resp = &pbmsg.SendMsgResp{
			ServerMsgID: req.MsgData.ServerMsgID,
			ClientMsgID: req.MsgData.ClientMsgID,
