...
func (m *conversationServer) GetConversationList(ctx context.Context, req *pbconversation.GetConversationListReq) (resp *pbconversation.GetConversationListResp, err error) {
	opUserID := mcontext.GetOpUserID(ctx)
	if opUserID == "" {
		log.ZError(ctx, "[diag-conversation-auth] GetConversationList denied: opUserID empty (caller must pass OpUserID in RPC metadata)", nil,
			"reqUserID", req.GetUserID())
		return nil, errs.ErrNoPermission.Wrap("opUserID is empty")
	}
	if req.UserID == "" {
		req.UserID = opUserID
	}
	if err := authverify.CheckAccessV3(ctx, req.UserID); err != nil {
		log.ZError(ctx, "[diag-conversation-auth] GetConversationList denied: CheckAccessV3", err,
			"opUserID", opUserID, "reqUserID", req.GetUserID())
		return nil, err
	}
...
func (c *conversationServer) GetAllConversations(ctx context.Context, req *pbconversation.GetAllConversationsReq) (*pbconversation.GetAllConversationsResp, error) {
	opUserID := mcontext.GetOpUserID(ctx)
	if opUserID == "" {
		log.ZError(ctx, "[diag-conversation-auth] GetAllConversations denied: opUserID empty (caller must pass OpUserID in RPC metadata)", nil,
			"reqOwnerUserID", req.GetOwnerUserID())
		return nil, errs.ErrNoPermission.Wrap("opUserID is empty")
	}
	if req.OwnerUserID == "" {
		req.OwnerUserID = opUserID
	...
	if err := authverify.CheckAccessV3(ctx, req.OwnerUserID); err != nil {
		log.ZError(ctx, "[diag-conversation-auth] GetAllConversations denied: CheckAccessV3", err,
			"opUserID", opUserID, "reqOwnerUserID", req.GetOwnerUserID())
		return nil, err
	}
