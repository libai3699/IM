...
func (m *MessageApi) GetActiveUser(c *gin.Context) {
	// ✅ 权限检查：只允许管理员调用此接口
	if err := authverify.CheckAdmin(c); err != nil {
		apiresp.GinError(c, errs.ErrNoPermission.Wrap("only admin can get active users"))
		return
	}
...
func (m *MessageApi) GetActiveGroup(c *gin.Context) {
	// ✅ 权限检查：只允许管理员调用此接口
	if err := authverify.CheckAdmin(c); err != nil {
		apiresp.GinError(c, errs.ErrNoPermission.Wrap("only admin can get active groups"))
		return
	}
...
func (m *MessageApi) SearchMsg(c *gin.Context) {
	// ✅ 权限检查：只允许管理员调用此接口
	if err := authverify.CheckAdmin(c); err != nil {
		apiresp.GinError(c, errs.ErrNoPermission.Wrap("only admin can search messages"))
		return
	}
