...
# Check and install protoc if needed
if ! command -v protoc &> /dev/null; then
    ...
    unzip -o -q ${PROTOC_ZIP} -d /usr/local
...
# First generate proto files
echo "==> Generating proto files..."
cd $OPENIM_ROOT/pkg/proto
bash gen.sh
cd $OPENIM_ROOT

if [ $? -ne 0 ]; then
  echo "proto generation Error, script exits"
  exit 1
fi
