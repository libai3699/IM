...
// escapeLikeKeyword 转义 LIKE 通配符，避免 keyword 为 % 时生成 %% 匹配全表。
...
// IsInvalidSearchKeyword 纯通配符关键字会导致 LIKE 匹配全部记录。
func IsInvalidSearchKeyword(keyword string) bool {
	return strings.TrimSpace(keyword) != "" && strings.Trim(keyword, "%_") == ""
}
...
func (o *Attribute) Search(ctx context.Context, keyword string, genders []int32, page int32, size int32) (uint32, []*chat.Attribute, error) {
	...
	if keyword != "" {
		db = db.Where("`user_id` = ? OR `account` = ? OR `phone_number` = ?", keyword, keyword, keyword)
	}
	return ormutil.GormPage[chat.Attribute](db, page, size)
}
...
func (o *Attribute) SearchNormalUser(ctx context.Context, keyword string, forbiddenIDs []string, gender int32, page int32, size int32, fuzzy bool) (uint32, []*chat.Attribute, error) {
	db := o.db.WithContext(ctx)
	var genders []int32
	if gender == 0 {
	...
	if keyword != "" {
		if fuzzy {
			kw := escapeLikeKeyword(keyword)
			db = db.Where("`user_id` LIKE ? ESCAPE '\\\\' OR `account` LIKE ? ESCAPE '\\\\' OR `phone_number` LIKE ? ESCAPE '\\\\' OR `nickname` LIKE ? ESCAPE '\\\\'", kw, kw, kw, kw)
		} else {
			kw := escapeLikeKeyword(keyword)
			db = db.Where("`account` LIKE ? ESCAPE '\\\\' OR `phone_number` = ?", kw, keyword)
		}
	}
	return ormutil.GormPage[chat.Attribute](db, page, size)
}
...
func (o *Attribute) SearchUser(ctx context.Context, keyword string, userIDs []string, genders []int32, pageNumber int32, showNumber int32) (uint32, []*chat.Attribute, error) {
	...
	if keyword != "" {
		kw := escapeLikeKeyword(keyword)
		db = db.Where("`account` LIKE ? ESCAPE '\\\\' OR `phone_number` LIKE ? ESCAPE '\\\\'", kw, kw)
	}
	return ormutil.GormPage[chat.Attribute](db, pageNumber, showNumber)
}
