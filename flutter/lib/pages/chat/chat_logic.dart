...
class ChatLogic extends GetxController {
  ...
  /// 一键屏幕共享：创建快速会议并邀请当前会话对象加入
  Future<void> onTapScreenShare() async {
    if (rtcIsBusy) {
    ...
    try {
      // 1) 创建快速会议
      final meetingName = sprintf(StrRes.meetingInitiatorIs, [OpenIM.iMManager.userInfo.nickname]);
      final start = DateTime.now().millisecondsSinceEpoch ~/ 1000;
      final duration = 60 * 60; // 1小时
      final cert = await LoadingView.singleton.wrap(
      ...
      await MeetingClient().join(
        Get.context!,
        meetingID: cert.roomID!,
        meetingName: meetingName,
        participantNickname: OpenIM.iMManager.userInfo.nickname,
      );

      // 3) 邀请当前聊天对象加入（单聊或群聊）
      await MeetingClient().invite(
        meetingID: cert.roomID!,
        meetingName: meetingName,
        startTime: start,
        duration: duration,
      ...
      // 4) 友好提示
      IMViews.showToast(StrRes.invitesYouToVideoConference.replaceFirst('%s', OpenIM.iMManager.userInfo.getShowName()));
    } catch (e, s) {
      // Logger.print('onTapScreenShare error: $e $s');
