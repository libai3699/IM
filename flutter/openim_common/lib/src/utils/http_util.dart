...
class HttpUtil {
  ...
  static Future post(
    ...
    try {
      ...
      // Logger.print('HTTP请求 baseUrl: ${dio.options.baseUrl}');
  ...
  static Future<String> uploadImageForMinio({
    ...
    final uploadUrl = "${Config.imApiUrl}/third/minio_upload";
    Logger.print('[图片上传] 开始上传图片');
    Logger.print('[图片上传] 使用的域名: ${Config.imApiUrl}');
    Logger.print('[图片上传] 完整上传URL: $uploadUrl');
    ...
    final imageUrl = resp.data?['data']['URL'];
    Logger.print('[图片上传] 上传成功，返回的图片URL: $imageUrl');
