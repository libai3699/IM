...
class DataSp {
  ...
  static const _currentLineIndex = 'currentLineIndex';
  static const _dynamicLineConfig = 'dynamicLineConfig';
  ...
  /// 保存当前线路索引
  static Future<bool>? putCurrentLineIndex(int index) {
    return SpUtil().putInt(_currentLineIndex, index);
  }

  /// 获取当前线路索引
  static int? getCurrentLineIndex() {
    return SpUtil().getInt(_currentLineIndex);
  }

  /// 保存动态线路配置
  static Future<bool>? putDynamicLineConfig(List<Map<String, String>> lines) {
    final jsonString = jsonEncode(lines);
    return SpUtil().putString(_dynamicLineConfig, jsonString);
  }

  /// 获取动态线路配置
  static List<Map<String, String>> getDynamicLineConfig() {
    final jsonString = SpUtil().getString(_dynamicLineConfig);
    if (jsonString != null && jsonString.isNotEmpty) {
      try {
        final List<dynamic> jsonList = jsonDecode(jsonString);
        return jsonList.map((item) => Map<String, String>.from(item)).toList();
      } catch (e) {
        // Logger.print('解析动态线路配置失败: $e');
      }
    }
    return [];
  }
