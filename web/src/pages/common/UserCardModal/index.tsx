...
> = (props, ref) => {
  ...
  const getCardInfo = async (): Promise<CardInfo | null> => {
    ...
    if (data[0]?.groupMemberInfo) {
      setGroupMemberInfoRow(data[0]?.groupMemberInfo);
    } else {
      setGorupMemberFields([]);
    }
  ...
  const showSendMessage = !isSelf && (isFriend || allowSendMsgNotFriend);
  ...
  return (
    ...
    >
          ...
          <div className="flex max-h-[520px] min-h-[484px] flex-col overflow-hidden bg-[url(@/assets/images/common/card_bg.png)] bg-[length:332px_134px] bg-no-repeat px-5.5">
            ...
            <div className="ignore-drag flex flex-1 flex-col overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                {/* 只有高级用户才能看到登录日志 */}
                {cardInfo?.userRole === "高级用户" && cardInfo?.userID && (
                  <div className="my-4">
                    <div className="mb-2 flex items-center justify-between">
                      <div className="text-[var(--sub-text)]">登录信息</div>
                      <Button 
                        type="link" 
                        size="small"
                        onClick={() => setShowLoginRecords(true)}
                      >
                        查看完整记录
                      </Button>
                    </div>
                    <UserLoginInfo userID={cardInfo.userID} />
      ...
      <Modal
        title="登录记录"
        open={showLoginRecords}
        onCancel={() => setShowLoginRecords(false)}
        footer={null}
        width={600}
        centered
      >
        {cardInfo?.userID && <UserLoginRecordList userID={cardInfo.userID} />}
      </Modal>
    </DraggableModalWrap>
  );
};
