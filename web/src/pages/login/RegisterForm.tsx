...
const RegisterForm = ({ loginMethod, setFormType }: RegisterFormProps) => {
  ...
  const needVerificationCode = useUserStore(
    (state) =>
      Number(state.appConfig.needVerificationCodeRegister) === BusinessAllowType.Allow,
  );
      ...
      >
        ...
        {needInvitationCode && (
          <Form.Item
            ...
            name="invitationCode"
            hidden={registerForm !== 0}
            rules={[{ required: true, message: t("toast.inputInvitationCode") }]}
          >
            <Input
              allowClear
              spellCheck={false}
