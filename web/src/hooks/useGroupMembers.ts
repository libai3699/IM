  ...
  const searchMember = useCallback(
    async (keyword: string) => {
      ...
      const currentConversationGroupID =
        useConversationStore.getState().currentConversation?.groupID;
      try {
        const actualKeyword = keyword === REACH_SEARCH_FLAG ? lastKeyword.current : keyword;
        const targetGroupID = groupID ?? currentConversationGroupID ?? "";
        const { data } = await IMSDK.searchGroupMembers({
          groupID: targetGroupID,
          offset: isReach ? latestFetchState.current.searchOffset : 0,
          count: 20,
          keywordList: [actualKeyword],
        ...
        let mergedData = data;
        if (!isReach) {
          const upperKeyword = actualKeyword.toUpperCase();
          const friendList = useContactStore.getState().friendList;
          const sdkUserIDs = new Set(data.map((m) => m.userID));
          const remarkMatchIDs = friendList
            .filter(
              (f) =>
                !sdkUserIDs.has(f.userID) &&
                f.remark?.toUpperCase().includes(upperKeyword),
            )
            .map((f) => f.userID);
          if (remarkMatchIDs.length > 0) {
            try {
              const { data: members } = await IMSDK.getSpecifiedGroupMembersInfo({
                groupID: targetGroupID,
                userIDList: remarkMatchIDs,
              });
              mergedData = [...data, ...members];
            } catch {
              // not in this group, ignore
            }
          }
        }
        ...
        setFetchState((state) => ({
          ...state,
          searchMemberList: [...(isReach ? state.searchMemberList : []), ...mergedData],
